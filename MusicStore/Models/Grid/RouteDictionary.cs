...
namespace MusicStore.Models
{
    ...
    public class RouteDictionary : Dictionary<string, string>
    {
        ...
        public string GenreFilter {
            get => Get(nameof(AlbumsGridDTO.Genre))?.Replace(FilterPrefix.Genre, "");
            set => this[nameof(AlbumsGridDTO.Genre)] = value;
        }

        public string PriceFilter {
            get => Get(nameof(AlbumsGridDTO.Price))?.Replace(FilterPrefix.Price, "");
            set => this[nameof(AlbumsGridDTO.Price)] = value;
        }

        public string ArtistFilter {
            get
            {
                string s = Get(nameof(AlbumsGridDTO.Artist))?.Replace(FilterPrefix.Artist, "");
                int index = s?.IndexOf('-') ?? -1;
                return (index == -1) ? s : s.Substring(0, index);
            }
            set => this[nameof(AlbumsGridDTO.Artist)] = value;
        }

        public void ClearFilters() =>
            GenreFilter = PriceFilter = ArtistFilter = AlbumsGridDTO.DefaultFilter;
